...
const BOT_TOKEN = "7620399081:AAHeCUimxg365BH0Y0P_aWoWl1MQF0r7DQw"; // Replace with your bot token
const ADMIN_ID = "7689032393"; // Replace with your Telegram ID
const bot = new Telegraf(BOT_TOKEN);

const sessions = {};

bot.start((ctx) => {
    ctx.reply("📂 Send me a .txt file containing phone numbers.");
});
...
async function processFile(ctx, session) {
    try {
        console.log("Fetching file from Telegram:", session.fileUrl);
        const response = await axios.get(session.fileUrl, { responseType: "text" });
        ...
        const outputDir = "/tmp"; // Use Railway's temporary storage
